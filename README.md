...
Again, it is an experiment in building and hosting an ultra-cheap website. The goal is to get it as close to free as possible, while using a solid development tool chain. I'll try keep this readme up to date with the latest tooling details.
...
- Netlify monitors Github, and kicks off a continuous integration to build and host
...
Summary of findings:
Statiq is different than Hugo in that, Hugo is a stand-alone program, while Statiq is a Nuget package (.dlls) within your solution. In Statiq, you add content to an "Input" folder, do a commandline "dotnet run" for the whole project, and it generates the output as part of the running program. 
...
There are advantages to both approaches, but I think one of the biggest, is that Statiq's version-control is built in (whatever you include in the project is what get built). You could even have multiple versions running in parallel. Where as, an update to your Hugo installation changes everything, and could potentially break the build. Secondarily, you can guarantee everyone on your team has the same version of Statiq, because there is no install. It a DLL that sits within your Source Control.
